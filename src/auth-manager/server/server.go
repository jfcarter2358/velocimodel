...
func (s *Server) GetAuthorizeData(rt oauth2.ResponseType, ti oauth2.TokenInfo, token string) map[string]interface{} {
	if rt == oauth2.Code {
...
func (s *Server) HandleAuthorizeRequest(w http.ResponseWriter, r *http.Request) error {
	...
	client, err := s.Manager.GetClient(ctx, req.ClientID)
	params := &oauth2.TokenGenerateRequest{
	...
	newti, err := s.Manager.GenerateAuthToken(ctx, oauth2.Token, params)
	return s.redirect(w, req, s.GetAuthorizeData(req.ResponseType, ti, newti.GetAccess()))
}
